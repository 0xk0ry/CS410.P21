...
def get_args():
    ...
    parser.add_argument('--out-dir', default='train_free_output',
...
def main():
    ...
    logfile = os.path.join(args.out_dir, f'cifar10_free1.log')
    ...
    csv_logfile = os.path.join(args.out_dir, 'cifar10_free_metrics.csv')
    ...
    for epoch in range(args.epochs):
        ...
        if (epoch + 1) % 5 == 0 or epoch == args.epochs - 1:
            checkpoint_path = os.path.join(
                args.out_dir, f'checkpoint_free_epoch_{epoch+1}.pth')
            torch.save({
                'epoch': epoch,
    ...
    torch.save(model.state_dict(), os.path.join(
        args.out_dir, f'train_free_output.pth'))
